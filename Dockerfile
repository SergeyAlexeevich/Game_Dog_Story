...
COPY --from=build /app/build/bin/catch_tests_loot_gen /app/
COPY --from=build /app/build/bin/collision_detection_tests /app/
COPY --from=build /app/build/bin/serialization_tests /app/
